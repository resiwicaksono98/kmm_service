        ...
        <div class="px-8">
            <form @submit.prevent="handleSubmit">
                ...
                <div class="mb-4">
                    ...
                    <button
                        type="submit"
                        class="mt-4 flex items-center gap-1 bg-primary text-white py-2 px-4 rounded-md hover:bg-secondary transition duration-300 text-sm"
...
const page = usePage();
const { errors, tasks } = defineProps({ errors: Object, tasks: Object });
...
const removeTask = (task, index) => {
    if (task.id) {
        const getTask = tasks.data.find((t) => t.id === task.id);
...
function handleSubmit() {
    form.post("/admin/packages/create", {
        name: form.name,
        image: form.image,
